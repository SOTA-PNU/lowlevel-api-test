...
def generate_report(results: Dict[str, TestResultInfo], args, triton_module, api) -> str:
    ...
    lines.append("-----------------")
    lines.append(f"{'name':42} {'module':10} {'dtype':22} {'exec':7} {'accuracy':8} {'ms':>10} {'GB/s':>10}    detail")
    lines.append("-" * 110)
    for name, r in sorted(results.items()):
        lines.append(f"{name:42} {r.module:10} {r.dtype:22} {r.exec_status:7} {r.accuracy_status:8} {_metric(r.ms, 4):>10} {_metric(r.gbps, 2):>10}    {_report_detail(r.detail)}")
