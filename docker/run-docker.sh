...
ensure_triton_build() {
    ...
    if docker run --rm \
        ...
        /opt/triton-venv/bin/python \
        -c "import triton; print('Triton is ready')" \
        > /dev/null 2>&1; then

        echo "Triton is available in the Docker image"
    else
        echo "Triton is not available in the Docker image."
        echo "Rebuild the image through stable_test.yml or docker build."
        exit 1
    fi
}
